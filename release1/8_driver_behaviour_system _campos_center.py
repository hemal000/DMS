...
mouth_threshold = 45  # mouth threshold value
eye_threshold = 4.0  # eye threshold value
y_rotation = 15  # yaw rotation along x axis threshold value
eye_closed_threshold = 4.6  # eye closed threshold value
######################################################################################################
...
while cap.isOpened():
    ...
    frame = cv2.cvtColor(cv2.flip(roi_frame, 1), cv2.COLOR_BGR2RGB)  # Flipping the frame horizontally and converting color space from BGR to RGB
    frame.flags.writeable = False   # Setting the writeable flag of the frame to False for improving performance
    results = face_mesh.process(frame)   # Processing the frame to detect facial landmarks using the face mesh model
    frame.flags.writeable = True   # Setting the writeable flag of the frame back to True 
    # convert from RGB to BGR
    frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)  # Converting the color space of the frame from RGB to BGR
    img_h, img_w, img_c = frame.shape  # Getting the height, width, and number of channels of the frame
    ...
    if results.multi_face_landmarks:
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            if y < -y_rotation or y > y_rotation:  # Checking if the rotation degree around the y-axis indicates distraction 
                is_distracted = True
                if time.time() - distraction_start_time >= distraction_time_duration:
                    ...
                    # countinue counting distraction time
                    pass
            else:
                is_distracted = False
                distraction_start_time = time.time()  # Reset the distraction start time
            # Display the timer on the screen
            dist_timer_text = "dist: {:.1f}s".format(distraction_time_duration - (time.time() - distraction_start_time))    
            cv2.putText(frame, f"Timer: {dist_timer_text}" , (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)  # Adding text to the frame
            ...
            # Check if there are multiple face landmarks detected in the results
            if results.multi_face_landmarks:
                mesh_coords = landmark_detection(frame, results, False)  # Detecting and drawing landmarks on the frame
                eye_dist = eye_aspect_ratio(frame, mesh_coords, RIGHT_EYE, LEFT_EYE)  # Calculating the eye aspect ratio
                mouth_dist = mouth_aspect_ratio(frame, mesh_coords, UPPER_LIPS, LOWER_LIPS)  # Calculating the mouth aspect ratio
                print(f"Eye_distance: {eye_dist}, Mouth_Dist: {mouth_dist}.")  # Printing Eye aspect ratio and Mouth aspect ratio

                if eye_dist > eye_threshold: # Checking if the eye aspect ratio indicates drowsiness
                    #cv2.putText(frame, "Drowsy", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)  # Adding text to the frame
                    text = "Drowsy"
                    is_drowsy = True
